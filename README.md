...
The app tries MP3 first, then falls back to WAV. The bundled WAV files are lightweight generated sound effects so the demo has sound immediately. The MP3 files are placeholders for your future polished assets.

To replace a sound, overwrite the matching MP3 file with a real MP3 using the same filename:
...
If the MP3 is unavailable or cannot play, the app uses:
...
The homepage Sound On / Off toggle controls all sounds.
